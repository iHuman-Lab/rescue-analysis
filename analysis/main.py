...
if __name__ == "__main__":
    ...
    if not preloaded:
        preloaded = load_all_subjects(
            cfg
        )  # xdf skipped → load once from data/intermediate
    ...
    with skip_run("run", "extract_features") as check, check():
        best_features = run_extract_features(
            cfg, preloaded, eyetracking, processed_dir=processed_dir
        )

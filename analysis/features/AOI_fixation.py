...
from analysis.features.conventions import DEFAULT_OFFSCREEN_LABEL, strip_run_suffix
...
def run_aoi_fixations(
    cfg: dict, eyetracking: dict | None = None, preloaded: dict | None = None
) -> dict:
    ...
    if not aois:
        print(
            "WARNING: no AOIs defined in config — all fixations will be labelled 'offscreen'"
        )
...
def aggregate_transitions(
    aoi_results: dict, aois: list[dict], cfg: dict
    ...
    for sid, trials in aoi_results.items():
        for trial_id, trial_data in trials.items():
            mat = trial_data["transitions"].reindex(
            ...
            by_trial[base] = (
                by_trial.get(base, pd.DataFrame(0, index=labels, columns=labels)) + mat
            )
